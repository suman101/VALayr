...
Validator component (`validator/utils/difficulty.py`) that controls progressive difficulty across epochs. Manages phase transitions (1→2→3), mainnet-to-synthetic task ratios, and minimum severity floors. Configured via `VALAYR_EPOCH_DIFFICULTY_*` environment variables.
...
The output of `close_epoch()` in the Orchestrator. Contains the epoch number, aggregated miner scores, weight vector, and submission statistics. Persisted to `data/epochs/epoch_<N>.json`.
...
The central registry (`task-generator/mutator/registry.py`) that composes and applies mutators in a deterministic order to produce task variants from templates.
...
Dataclass returned by `Orchestrator.process_submission()`. Contains validation result, severity score, fingerprint, dedup status, reward multiplier, and timing metadata.
...
Penalisation mechanism where a validator's stake or reputation is reduced for misbehaviour. In VALayr, validators with >20% divergence from consensus trigger divergence-based slashing via the anti-collusion engine.
...
Dataclass produced by the Task Generator. Contains the vulnerable Solidity source, deployment config, task metadata, and the expected vulnerability class. Used as input to the validation pipeline.
...
Validator component (`validator/scoring/uniqueness.py`) that computes structural similarity between exploit submissions to differentiate truly novel approaches from trivial variations of the same attack.
