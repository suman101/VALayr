...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| R-1 | ~~**Reentrancy in `payExploitReward` / `withdrawBounty`**~~ — `nonReentrant` added to ProtocolRegistry (`payExploitReward`, `withdrawBounty`, `withdrawBountyAll`) and Treasury (`withdrawPrize`, `withdrawFees`)                                                                                                              | ~~Add OpenZeppelin `ReentrancyGuard`~~                                                                                                                                                                                                                                    | ✅ Resolved                      |
| R-2 | **Centralised ownership** — all contracts use `Ownable2Step` (two-step transfer)                                                                                                                                                                                                                                               | Mitigated for testnet; production requires `TRANSFER_DELAY > 0` (48 h) and multi-sig before mainnet                                                                                                                                                                       | High — _mitigated (testnet)_     |
...
| R-7 | **`registerContract` front-running** — `registerContract()` is permissionless (anyone with `MIN_BOUNTY` can register). An attacker monitoring the mempool can front-run a legitimate protocol's registration TX with a higher gas price, squatting on the contract hash and collecting bounty rewards under their own address. | Implement a commit-reveal scheme: (1) `commitRegistration(hash(contractAddr, salt))` in TX-1, (2) `revealRegistration(contractAddr, salt)` in TX-2 after ≥ N blocks. This prevents mempool sniping. Alternatively, use Flashbots / private mempools for registration TXs. | High — _accepted risk (testnet)_ |
